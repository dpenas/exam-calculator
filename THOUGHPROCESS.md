- What kind of libraries/frameworks do I need?
    ...
    - These libraries can help in the beginning:
        ...
        - This library has the Pearson correlation coeficient (PCC) formula: https://github.com/markrogoyski/math-php, it seems active and there could be other useful functions. PHP includes stats_stat_correlation, but some people in the comments complained about getting errors. Might consider it (or just make the formula myself).
- Main Structure
...
- Improvements
    ...
    - Adding logs and sanitization (in cases the data might not be correct, files might not exist, some potential divisions by 0 in some other parts...).
    - I have repeated some logic in the tests. Might be nice having it somewhere else to avoid repeated code.
    - I would have liked having more unit tests for smaller functions. I considered using the big excel file as a test, but it's quite big and can take a while to process if it ends up running frequently.
    - I haven't put too much thought about public/protected/private functions. I have made everything public for now since it could be easier for testing, but with a bit more time I would have liked looking into this a bit more.
    - The export could highlight some values. For example, questions that might be too easy/too hard.